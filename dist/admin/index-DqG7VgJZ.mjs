...
const richTextField = {
  ...
  icon: Paragraph,
  components: {
    Input: async () => import("./RichTextInput-D6NFQkn3.mjs").then((m) => ({ default: m.default }))
  },
  options: {
    advanced: [
      {
  ...
  async registerTrads({ locales }) {
    return Promise.all(
      locales.map(async (locale) => {
        try {
          const { default: data } = await __variableDynamicImportRuntimeHelper(/* @__PURE__ */ Object.assign({ "./translations/en.json": () => import("./en-Cjkn__gZ.mjs") }), `./translations/${locale}.json`, 3);
          return { data, locale };
        } catch {
          return { data: {}, locale };
        }
